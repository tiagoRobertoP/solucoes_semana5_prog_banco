...
#exercicio 01 semana 05
for user in users:
    user["interessado em"] = []
    if user["id"] % 2 ==0 : 
        user["interessado em"]= "M"
        if user["id"] % 4 == 0 :
            user["interessado em"]= "MF"
    else:
        user["interessado em"] = "F"    

#print (users) 



#exercicio 02 semana 05
def interest_in_same (user):
    if user["interessado em"] == "M" :
        for user2 in users :
            if user2["interessado em"] == "M" :
                return user2["name"]
    elif user["interessado em"] == "F" :
        for user2 in users :
            if user2["interessado em"] == "F" :
                return user2["name"]
    elif user["interessado em"] == "MF" :
        for user2 in users :
            if user2["interessado em"] == "MF" :
                return user2["name"]
    else :
        interest_in_same (user)   


#print (interest_in_same (users[6]))
    

#exercicio 03 semana 05    
def interest_in_same2 (user):
    if user["interessado em"] == "M":
        for user2 in users :
            if user2["interessado em"] == "M":
                for user_id , interest in interests :
                    if user_id == user["id"] and user_id == user2["id"] :
                        return user2["name"]
    elif user["interessado em"] == "F" :
        for user2 in users :
            if user2["interessado em"] == "F" :
        ...
        interest_in_same2 (user)   


print (interest_in_same2 (users[2]))    